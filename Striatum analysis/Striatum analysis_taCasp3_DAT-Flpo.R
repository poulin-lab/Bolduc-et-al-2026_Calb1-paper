...
setwd("C:/Users/cyril/OneDrive - McGill University (1)/Documents/PhD/Raw data/Striatal spectra/DAT-2A-Flpo/CSVfiles")
...
for (i in 1:nfiles) {
  ...
  setwd("C:/Users/cyril/OneDrive - McGill University (1)/Documents/PhD/Raw data/Striatal spectra/DAT-2A-Flpo/Noise")  
  ...
  #Crop to get a number of pixels that is mulplicater of the number of box
...
df$Slice <- gsub("Slice1.tif.csv", "Slice1", df$Slice)
df$Slice <- gsub("Slice2.tif.csv", "Slice2", df$Slice)
df$Slice <- gsub("Slice3.tif.csv", "Slice3", df$Slice)
df$Slice <- gsub("Slice4.tif.csv", "Slice4", df$Slice)

#Extract the groups
NotaCasp3 <- df[df$Group == "NotaCasp3",]
CoffFontaCasp3 <- df[df$Group == "CoffFontaCasp3",]
ConFontaCasp3 <- df[df$Group == "ConFontaCasp3",]

#Extract the bacthes of immuno
Batch1_NotaCasp3 <- rbind(NotaCasp3[NotaCasp3$MouseID == c("G0482"),],
                          NotaCasp3[NotaCasp3$MouseID == c("G0483"),],
                          NotaCasp3[NotaCasp3$MouseID == c("G0489"),],
                          NotaCasp3[NotaCasp3$MouseID == c("G0487"),],
                          NotaCasp3[NotaCasp3$MouseID == c("G0517"),],
                          NotaCasp3[NotaCasp3$MouseID == c("G0568"),]
                          )

Batch2_NotaCasp3 <- rbind(NotaCasp3[NotaCasp3$MouseID == c("H0653"),],
                          NotaCasp3[NotaCasp3$MouseID == c("h0777"),],
                          NotaCasp3[NotaCasp3$MouseID == c("I0166"),],
                          NotaCasp3[NotaCasp3$MouseID == c("I0169"),]
                          )

Batch2_CoffFontaCasp3 <- rbind(CoffFontaCasp3[CoffFontaCasp3$MouseID == c("H0654"),],
                               CoffFontaCasp3[CoffFontaCasp3$MouseID == c("H0778"),],
                               CoffFontaCasp3[CoffFontaCasp3$MouseID == c("H0779"),],
                               CoffFontaCasp3[CoffFontaCasp3$MouseID == c("H0880"),],
                               CoffFontaCasp3[CoffFontaCasp3$MouseID == c("I0167"),],
                               CoffFontaCasp3[CoffFontaCasp3$MouseID == c("I0168"),]
)

#Extract the bacthes of immuno
Batch1_ConFontaCasp3 <- rbind(ConFontaCasp3[ConFontaCasp3$MouseID == c("G0485"),],
                              ConFontaCasp3[ConFontaCasp3$MouseID == c("G0490"),],
                              ConFontaCasp3[ConFontaCasp3$MouseID == c("G0488"),],
                              ConFontaCasp3[ConFontaCasp3$MouseID == c("G0495"),],
                              ConFontaCasp3[ConFontaCasp3$MouseID == c("G0497"),],
                              ConFontaCasp3[ConFontaCasp3$MouseID == c("G0554"),]
)

Batch1 <- rbind(Batch1_NotaCasp3, Batch1_ConFontaCasp3)
...
#I'll exclude the slice2 of G0554, since it had a damage and was interfering with the MV-DL axis
df <- df[!(df$MouseID == "G0554" & df$Slice == "Slice2"),]
#------------------------------------------------------------------------------------
...
average_on_CTL <- function(Batch, Channel, Slice, Striatal_Axis) {
  ...
  for (x in 1:number_of_box) {
    temp <- df[df$Immuno_Batch == Batch & df$Channel == Channel & df$Group == "NotaCasp3" & df$Slice == Slice & df$Striatal_Axis == Striatal_Axis,7:index]
...
#For simplicity of the code, just substitute the IC function by SEM function, so I won't replace every Confidence_Interval_95 in the following lines
...
Average <- Normalized_Curves_Value[Normalized_Curves_Value$Group == "NotaCasp3" &
...
up <- Normalized_Curves_IC95_max[Normalized_Curves_IC95_max$Group == "NotaCasp3" &
...
low <- Normalized_Curves_IC95_min[Normalized_Curves_IC95_min$Group == "NotaCasp3" &
...
lol <- normalized_df[normalized_df$Group == "ConFontaCasp3" & normalized_df$Slice == "Slice3" & normalized_df$Channel == "TH" & normalized_df$Striatal_Axis == "MV-DL", ] 

View(normalized_df)

graph_individual_mouseID(MouseID = "G0497",
                         Slice = "Slice3",
                         Channel = "TH",
                         Striatal_Axis = "DV")
#------------------------------------------------------------------------------------
...
Slice_lol <- "Slice2"
  ...
  a <- graph(Slice = Slice_lol,
             Channel = "mCherry",
             Striatal_Axis = "DV")
  
  b <- graph(Slice = Slice_lol,
             Channel = "mCherry",
             Striatal_Axis = "ML")
  
  c <- graph(Slice = Slice_lol,
             Channel = "mCherry",
             Striatal_Axis = "MV-DL")
  
  
  
  d <- graph(Slice = Slice_lol,
             Channel = "TH",
             Striatal_Axis = "DV")
  
  e <- graph(Slice = Slice_lol,
             Channel = "TH",
             Striatal_Axis = "ML")
  
  f <- graph(Slice = Slice_lol,
             Channel = "TH",
             Striatal_Axis = "MV-DL")
...
setwd("C:/Users/cyril/OneDrive - McGill University (1)/Documents/PhD/Raw data/Striatal spectra/DAT-2A-Flpo/Stats")
