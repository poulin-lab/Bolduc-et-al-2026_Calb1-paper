...
Groups <- c("CoffFontaCasp3", "NotaCasp3")
...
Average_Bout_Curve$Frame <- All_Bouts[[2]][[1]]$Frame
...
Average_Bout_Curve <- data.frame(rep(Average_Bout_Curve[,"Frame"], times=2), 
                                 c(Average_Bout_Curve[,"CoffFontaCasp3"],
                                   Average_Bout_Curve[,"NotaCasp3"]),
...
ggplot(Average_Bout_Curve, aes(x=Frame, y=Acceleration_Value, color = Group)) +
  ...
  ylim(-300,500) +
  scale_color_manual(values = c("CoffFontaCasp3" = "red", "NotaCasp3" = "black")) +
...
for (i in 1:length(Bouts_Frequency)) {
  ...
  for  (x in 1:length(All_Bouts[[i]])) {
    ...
    Peak_Speed[Peak_Speed > 400] <- 400
    Duration <- Duration[!is.na(Duration)]
    Duration[Duration > 3] <- 3
    Peak_Acceleration <- Peak_Acceleration[!is.na(Peak_Acceleration)]
    Peak_Acceleration[Peak_Acceleration > 2000] <- 2000 #Assign the acceleration values higher than 2000 to 3000
    Peak_Deceleration <- Peak_Deceleration[!is.na(Peak_Deceleration)]
    Peak_Deceleration[Peak_Deceleration < -2000] <- -2000 #Assign the acceleration values lower than -2000 to 3000
  ...
  Peak_Speed_Hist <- hist(Peak_Speed, breaks = seq(0, 400, length.out = 31), ylim = c(0,100))
  ...
  Peak_Acceleration_Hist <- hist(Peak_Acceleration, breaks = seq(0, 2000, length.out = 16), ylim = c(0,100))
  ...
  Peak_Deceleration_Hist <- hist(Peak_Deceleration, breaks = seq(0, -2000, length.out = 16), ylim = c(0,100))
...
Average_Occurence <- function(Parameter, Range) {
  ...
  Occurence_Curve <- data.frame(rep(df[,1], times=2), 
                                c(df[,2],
                                  df[,3]),
...
MouseIDs <- c("F658", "F706", "F708", "G0218", "G0219", "G0351", "I0032", "I0138", "I0212", "I0213", "I0248", "I0256", "F574", "F707", "F795", "G0215", "G0217", "G0262", "G0350", "I0029", "I0031", "I0135", "I0137", "I0214", "I0250")

#Compute the cumulative occurence for Peak Speed
Peak_Speed <- Average_Occurence(Parameter = "Peak Speed", Range = c(0,1))
temp <- t(rbind(Peak_Speed[[1]],Peak_Speed[[2]]))
...
temp <- t(rbind(Duration[[1]],Duration[[2]]))
colnames(temp) <- MouseIDs
data <- temp
for (x in 1:nrow(data)) {
  for (y in 1:ncol(data)) {
...
temp <- t(rbind(Peak_Acceleration[[1]],Peak_Acceleration[[2]]))
colnames(temp) <- MouseIDs
data <- temp
for (x in 1:nrow(data)) {
  for (y in 1:ncol(data)) {
...
temp <- t(rbind(Peak_Deceleration[[1]],Peak_Deceleration[[2]]))
colnames(temp) <- MouseIDs
...
p <- p +
  geom_ribbon(data = data2, aes(x = X, ymin = sem_min, ymax = sem_max), fill = "black", alpha = 0.3) +
  geom_line(data = data2, aes(x = X, y = Average), color = adjustcolor("black", alpha.f = 1), size = 0.5) 
...
Cumulative_Occurence_Plot(File = "Peak_Speed_Cumulative_Occurrence.csv",
                          ...
                          Group1_Col_Index = 2:13,
                          Group2 = "NotaCasp3",
                          Group2_Col_Index = 14:20
                        )
