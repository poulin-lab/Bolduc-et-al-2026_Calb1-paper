...
#Note: The value I got for this batch one pixel is equivalent to 0.0428cm, or 0.428mm
Resolution <- 0.428
...
saveRDS(Bouts_Amplitude, "BoutsAmplitude_DREADD_Batch1.rds")

#------------------------------------------------------------------------
setwd("C:/Users/cyril/OneDrive - McGill University (1)/Documents/PhD/Raw data/OpenField_Velocity Analysis/DREADD/Batch1/Output")

#Read the lists and combine them together
Batch1 <- readRDS("BoutsAmplitude_DREADD_Batch1.rds")
Batch2 <- readRDS("BoutsAmplitude_DREADD_Batch2.rds")
Bouts_Amplitude <- c(Batch1, Batch2)
...
Bouts_Correlation$Group

Bouts_Correlation$Group <- factor(Bouts_Correlation$Group, levels = c("fDIO-mCherry", "CoffFon-DREADD", "ConFon-DREADD"))
...
ggplot(Bouts_Correlation, aes(Time, MaxSpeed, colour = Group)) +
  ...
  stat_cor(method = "pearson", 
           ...
           size = 4, 
           show.legend = FALSE) +
  scale_color_manual(values = c("fDIO-mCherry" = "#000000", "CoffFon-DREADD" = "red", "ConFon-DREADD" = "#008A07")) +
...
ggplot(Bouts_Correlation, aes(Time, MaxAcceleration, colour = Group)) +
  ...
  stat_cor(method = "pearson", 
           ...
           size = 4, 
           show.legend = FALSE) +
  scale_color_manual(values = c("fDIO-mCherry" = "#000000", "CoffFon-DREADD" = "#FF0700", "ConFon-DREADD" = "#008A07")) +
...
write.csv(Corr_to_time, "DREADD_PearsonCoeff_Time_vs_VelAcc")
write.csv(pValue, "DREADD_pValueCorr_Time_vs_VelAcc")
