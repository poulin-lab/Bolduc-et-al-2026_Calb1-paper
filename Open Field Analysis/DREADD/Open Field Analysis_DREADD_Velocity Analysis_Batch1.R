...
#------------------------------------------------------------------------
# Extract the bouts 
#Note: Bouts will be defined as speed is defined as an occurence where the speed is suddenly increased >x mm/s for at least y ms 

#Set the threshold paramaters for the bouts
Speed_Threshold <- 25 #bout minimal speed in mm/s
Time_Threshold <- 0.6 #bout minimal time in sec.

#Extract the bouts
temp <- data$Speed
temp[temp < Speed_Threshold] <- 0 #Assign to 0 all speed values lower to 0 to extract the bouts
...
for (x in 1:(length(index)-1)) {
  ...
  Start <- temp[1, "Frame"]
  Bout_end <- Start + Time_Threshold #Estimate where the 600ms ends
  
  End <- Closest(temp$Frame, Bout_end, which = FALSE, na.rm = FALSE) #Get the closest value to that end of 600ms
...
write.csv(Velocity_Data,"Velocity_Data_Batch1.csv", row.names = FALSE)
