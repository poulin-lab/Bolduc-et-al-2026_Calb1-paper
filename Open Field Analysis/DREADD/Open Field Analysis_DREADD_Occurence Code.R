...
Groups <- c("CoffFon-DREADD", "ConFon-DREADD", "fDIO-mCherry")
...
Average_Bout_Curve <- data.frame(rep(Average_Bout_Curve[,1], times=3), 
                                 ...
                                 c(rep("CoffFon-DREADD", times = 41),
                                   rep("ConFon-DREADD", times = 41),
                                   rep("fDIO-mCherry", times = 41)))
...
ggplot(Average_Bout_Curve, aes(x=Frame, y=Acceleration_Value, color = Group)) +
  ...
  ylim(-300,500) +
  scale_color_manual(values = c("CoffFon-DREADD" = "red", "ConFon-DREADD" = "chartreuse4", "fDIO-mCherry" = "black")) +
...
Bouts_HeatMap <- function(Group, Random_Bouts_Number) { 
  ...
  max <- as.vector(apply(Combined_Bouts[,10:30], MARGIN = 1, FUN = mean))
  ...
  levelplot(New_Combined_Bouts, col.regions = paletteer_c("viridis::inferno", 100),
            cuts = 100, pretty = TRUE, labels = TRUE, at=seq(0,250,length=100), useRaster = TRUE)
  
}

Bouts_HeatMap(Group = "fDIO-mCherry", Random_Bouts_Number = 150)

Bouts_HeatMap(Group = "CoffFon-DREADD", Random_Bouts_Number = 150)

Bouts_HeatMap(Group = "ConFon-DREADD", Random_Bouts_Number = 150)
...
for (i in 1:length(Bouts_Frequency)) {
  ...
  for  (x in 1:length(All_Bouts[[i]])) {
    ...
    Peak_Speed[Peak_Speed > 500] <- 500
    Duration <- Duration[!is.na(Duration)]
    Duration[Duration > 3] <- 3
    Peak_Acceleration <- Peak_Acceleration[!is.na(Peak_Acceleration)]
    Peak_Acceleration[Peak_Acceleration > 2500] <- 2500 #Assign the acceleration values higher than 2000 to 3000
    Peak_Deceleration <- Peak_Deceleration[!is.na(Peak_Deceleration)]
    Peak_Deceleration[Peak_Deceleration < -2500] <- -2500 #Assign the acceleration values lower than -2000 to 3000
  ...
  Peak_Speed_Hist <- hist(Peak_Speed, breaks = seq(0, 500, length.out = 31), ylim = c(0,100))
  ...
  Peak_Acceleration_Hist <- hist(Peak_Acceleration, breaks = seq(0, 2500, length.out = 16), ylim = c(0,100))
  ...
  Peak_Deceleration_Hist <- hist(Peak_Deceleration, breaks = seq(0, -2500, length.out = 16), ylim = c(0,100))
...
Average_Occurence <- function(Parameter, Range) {
  ...
  Occurence_Curve <- data.frame(rep(df[,1], times=3), 
                                c(df[,2],
                                  df[,3],
                                  df[,4]),
                                c(rep("CoffFon-DREADD", times = nrow(df)),
                                  rep("ConFon-DREADD", times = nrow(df)),
                                  rep("fDIO-mCherry", times = nrow(df))))
...
Cumulative_Occurence_Plot(File = "Peak_Speed_Cumulative_Occurrence.csv",
                          ...
                          xlim = xlim(0,500),
                          ylim = ylim(0,1),
                          Group1 = "CoffFon-DREADD", 
                          Group1_Col_Index = 2:10,
                          Group2 = "ConFon-DREADD",
                          Group2_Col_Index = 11:18,
                          Group3 = "NotaCasp3",
                          Group3_Col_Index = 19:26)
