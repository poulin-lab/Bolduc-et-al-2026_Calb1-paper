...
#Note: The value I got for this batch one pixel is equivalent to 0.0464cm, or 0.464mm
Resolution <- 0.464
...
Bouts_Correlation$Group <- factor(Bouts_Correlation$Group, levels = c("NotaCasp3", "CoffFontaCasp3", "ConFontaCasp3"))
...
ggplot(Bouts_Correlation, aes(Time, MaxSpeed, colour = Group)) +
  ...
  stat_cor(method = "pearson", 
           ...
           size = 4, 
           show.legend = FALSE) +
  scale_color_manual(values = c("NotaCasp3" = "#000000", "CoffFontaCasp3" = "#FF0700", "ConFontaCasp3" = "#008A07")) +
...
write.csv(Corr_to_time, "PearsonCoeff_Time_vs_VelAcc")
write.csv(pValue, "pValueCorr_Time_vs_VelAcc")
